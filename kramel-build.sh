...
DEFCONFIG=vendor/arrow_alioth_defconfig
...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
	...
	then
		...
		git clone --depth=1 https://gitlab.com/Panchajanya1999/azure-clang clang-llvm
...
build_kernel() {
	...
	then
		...
		git commit -m "$DEFCONFIG: Regenerate configs
	...
	then
		MAKE+=(
			...
			LOCALVERSION=-${VERSION}-${DEVICE}-${DATE} \
			CC=clang \
			AR=llvm-ar \
...
gen_zip() {
	...
	cdir AnyKernel3
	zip -r $ZIPNAME-$DEVICE-$VERSION-$DATE . -x ".git*" -x "README.md" -x "*.zip"

	## Prepare a final zip variable
	ZIP_FINAL="$ZIPNAME-$DEVICE-$VERSION-$DATE"
