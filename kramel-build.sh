...
curl -s -X POST https://api.telegram.org/bot${TOKEN}/sendMessage -d text="Whutshup guys my kernel for ${BRANCH} build has started" -d chat_id=${chat_id} -d parse_mode=HTML
...
cp ${DTBO} $(pwd)/AnyKernel3
        ...
        curl -F chat_id="${chat_id}"  \
                    ...
                    -F document=@"$(pwd)/HyperX-${TANGGAL}.zip" \
